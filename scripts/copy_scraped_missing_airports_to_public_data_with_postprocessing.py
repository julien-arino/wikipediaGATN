...
def get_city_name(data):
    ...
    if data.get("city-served-wikipedia"):
        return data["city-served-wikipedia"]
    if data.get("city-served"):
        return data["city-served"]
...
def symmetrise_link(target_code, missing_data, route_airlines, is_cargo=False):
    ...
    airlines_dest_key = (
        "airlines_destinations_cargo" if is_cargo else "airlines_destinations"
    )

    if dest_key not in target_data or not isinstance(target_data[dest_key], list):
        target_data[dest_key] = []
    if airlines_dest_key not in target_data or not isinstance(
        target_data[airlines_dest_key], dict
    ):
        target_data[airlines_dest_key] = {}

    missing_code = (
        missing_data.get("iata") or missing_data.get("icao") or missing_data.get("gps")
    )
    if not missing_code:
        return False
    ...
    if not already_exists:
        ...
        print(
            f"    [+] Added {missing_code} ({missing_city}) to {target_code}'s {dest_key}"
        )
...
def main():
    ...
    for fname in sorted(files):
        ...
        for d in missing_data.get("destinations", []):
            ...
            if dest_code:
                symmetrise_link(
                    dest_code, missing_data, d.get("airlines", []), is_cargo=False
                )
        ...
        for d in missing_data.get("destinations_cargo", []):
            ...
            if dest_code:
                symmetrise_link(
                    dest_code, missing_data, d.get("airlines", []), is_cargo=True
                )
