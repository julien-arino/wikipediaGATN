...
if __name__ == "__main__":
    ...
    active_missing_csv = find_active_missing_airports(
        input_csv=raw_missing_csv, max_workers=10
    )

    print(f"\nDone! Active missing airports list saved to: {active_missing_csv}")
    print(
        "You can now run 'python scripts/scrape_missing_airports.py' to recover them."
    )
