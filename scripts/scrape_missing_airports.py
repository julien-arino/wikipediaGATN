...
def scrape_missing_airports():
                ...
                try:
                    with open(os.path.join(airport_data_dir, fname), "r", encoding="utf-8") as f:
                        ...
                        if data.get("wikipedia_url"):
                            known_public_urls.add(urllib.parse.unquote(data["wikipedia_url"]))
                except Exception:
                    pass
                    
    print(f"Loaded {len(known_public_urls)} existing airport URLs from public database.")
    ...
    with open(input_csv, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if link:
                code = row.get("iata_code", "").strip() or row.get("icao_code", "").strip() or row.get("ident", "").strip()
    ...
    for seed_url, forced_code in initial_urls:
        ...
        while queue:
            ...
            if not code:
                print(f"  -> Skipping: No valid IATA/ICAO/GPS code in ourairports.csv for {canonical_url}")
                continue
                
            # Check if ANY of the available codes are already processed
            already_processed = False
            for c in [iata, icao, gps]:
                if not c or c in ["iata code not found", "icao code not found", "gps code not found"]:
                    continue
                
                # Check public database
                if os.path.exists(os.path.join(airport_data_dir, f"{c}.json")):
                    print(f"  -> Skipping: {c} is already in the main airport database.")
                    already_processed = True
                    break
                    
                # Check current missing sweep (any level)
                existing_sweep_files = [f for f in os.listdir(output_dir) if f.startswith(f"{c}.")]
                if existing_sweep_files:
                    print(f"  -> Skipping: {c} already processed in current sweep at a different level.")
                    already_processed = True
                    break
            ...
            destinations_cargo = format_destinations_list(info.get("destinations_cargo", []), info.get("airlines_destinations_cargo", {}), url_map)
            ...
            saved_count += 1
            print(f"  -> Saved {fname} ({len(destinations)} pax dests, {len(destinations_cargo)} cargo dests)")
