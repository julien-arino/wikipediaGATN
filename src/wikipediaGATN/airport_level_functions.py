...
def fetch_wikipedia_airport_link(identifier: str, verbose: bool = False):
    ...
    if isinstance(identifier, str) and identifier.startswith("http"):
        ...
        if not m:
            warnings.warn(
                f"Invalid Wikipedia URL: {identifier!r}", UserWarning, stacklevel=2
            )
            return None
    ...
    # 3. Free-text name
        ...
        search_term = (
            identifier if "airport" in identifier.lower() else f"{identifier} airport"
        )
    ...
    try:
        ...
        logger.warning(
            "Could not parse search response for %r", identifier, exc_info=exc
        )
        return None

    if not results:
        warnings.warn(
            f"No Wikipedia page found for {identifier!r}.", UserWarning, stacklevel=2
        )
        return None
...
def fetch_wikipedia_airport_html(link: str, verbose: bool = False):
    ...
    try:
        ...
        if not html:
            warnings.warn(
                f"No HTML content returned for {page_title!r}.",
                UserWarning,
                stacklevel=2,
            )
        elif verbose:
    ...
    try:
        ...
        for page in pages.values():
            ...
            if revisions:
                ...
                if verbose:
                    print(
                        f"Fetched wikitext for {page_title!r} ({len(wikitext or ''):,} chars, edit: {timestamp})"
                    )
                return wikitext, timestamp
        warnings.warn(
            f"No wikitext revisions found for {page_title!r}.",
            UserWarning,
            stacklevel=2,
        )
        return None, None
        ...
        logger.warning(
            "Could not parse wikitext response for %r", page_title, exc_info=exc
        )
        return None, None
...
def _fetch_html_if_needed(identifier, link, html_content, verbose):
    ...
    if not link:
        warnings.warn(
            f"Could not resolve Wikipedia link for {identifier!r}.",
            UserWarning,
            stacklevel=3,
        )
        return None, None
...
def fetch_wikipedia_airlines(
    ...
    """
    if soup is None:
        link, html_content = _fetch_html_if_needed(
            identifier, link, html_content, verbose
        )
        if not html_content:
            return set()
...
def fetch_wikipedia_airlines_destinations(
    ...
    for header in soup.find_all(["h2", "h3", "h4"]):
        ...
        for tbl in header.find_all_next("table"):
            ...
            if prev_level_header and prev_level_header.get_text(
                strip=True
            ...
            is_cargo = (
                prev_header and "cargo" in prev_header.get_text(strip=True).lower()
            )
            ...
            airline_idx = next((i for i, th in enumerate(ths) if "airline" in th), None)
            dest_idx = next(
                (i for i, th in enumerate(ths) if "destination" in th), None
            )
            ...
            if airline_idx is None or dest_idx is None:
                ...
                for row in tbl.find_all("tr")[1:]:
                    ...
                    airline_names = [
                        a.get("title")
                        for a in cells[airline_idx].find_all("a")
                        if a.get("title")
                    ...
                    dest_names = [
                        a.get("title")
                        for a in cells[dest_idx].find_all("a")
                        if a.get("title")
                        and not a.get("href", "").startswith(
                            (
                                "/wiki/Wikipedia:",
...
def fetch_wikipedia_airport_info(
    ...
    if not infobox:
        warnings.warn(
            f"Could not parse infobox for {link!r}.", UserWarning, stacklevel=2
        )
        return {
            **_EMPTY,
            "wikipedia_url": link,
    ...
    info: dict = {
        ...
        "lon": infobox.get("lon"),
        "altitude": (
            infobox.get("elevation-m") or _feet_to_metres(infobox.get("elevation-f"))
        ),
...
def _record_progress(
    output_dir: str, iata_icao_gps: str, url: str, iata_icao_gps_from: str = ""
) -> None:
    ...
    with open(csv_path, "a", encoding="utf-8", newline="") as fh:
        ...
        writer.writerow(
            {
                "iata_icao_gps": iata_icao_gps,
                "url": url,
                "iata_icao_gps_from": iata_icao_gps_from,
            }
        )
...
def parse_infobox_from_wikitext(wikitext: str, verbose: bool = False) -> dict:
        ...
        if key.lower() == "coordinates":
            ...
            if coord_m:
                ...
                try:
                    ...
                    if len(args) >= 8:
                        lat = (
                            float(args[0]) + float(args[1]) / 60 + float(args[2]) / 3600
                        ) * (1 if args[3].upper() == "N" else -1)
                        lon = (
                            float(args[4]) + float(args[5]) / 60 + float(args[6]) / 3600
                        ) * (1 if args[7].upper() == "E" else -1)
                    elif len(args) >= 6:
                        lat = (float(args[0]) + float(args[1]) / 60) * (
                            1 if args[2].upper() == "N" else -1
                        )
                        lon = (float(args[3]) + float(args[4]) / 60) * (
                            1 if args[5].upper() == "E" else -1
                        )
                    elif len(args) >= 4:
    ...
    infobox_data = {
        ...
        if v
        and not (str(v).strip().startswith("<!--") and str(v).strip().endswith("-->"))
    }
...
def parse_wikitext_airlines_destinations(wikitext: str) -> dict:
    ...
    for template in wikicode.filter_templates():
        ...
        if not (
            t_name.startswith("airport dest list")
            or t_name.startswith("airport destination list")
        ):
            continue
        ...
        step = 2
        if template.has("3rdcoltitle"):
            step += 1
        if template.has("4thcoltitle"):
            step += 1
        if template.has("5thcoltitle"):
            step += 1
        if template.has("6thcoltitle"):
            step += 1
        ...
        for i in range(0, len(positional) - (step - 1), step):
            ...
            if dest_objs:
                ...
                target_dict[airline].extend(
                    [d for d in dest_objs if d["wikipedia_url"] not in existing_urls]
                )
...
def build_url_to_codes_map(verbose: bool = False) -> dict:
        ...
        with open(csv_path, "r", encoding="utf-8") as f:
            for row in csv.DictReader(f):
                ...
                if row.get("url") and code:
                    url_to_codes[urllib.parse.unquote(row["url"])] = {
                        "iata": code,
                        "icao": "icao code not found",
                    }
        ...
        for url, data in ourairports_data.items():
            if url:
                ...
                if parsed_url not in url_to_codes:
                    ...
                    if (
                        not gps_code
                        and not data.get("iata_code")
                        and not data.get("icao_code")
                    ):
    ...
    for i in range(0, len(urls_to_resolve), 50):
        ...
        try:
            r = requests.get(
                f"https://en.wikipedia.org/w/api.php?action=query&titles={titles_str}&redirects=1&format=json",
                headers=headers,
                timeout=10,
            )
            if r.status_code == 200:
                ...
                if "query" in res_json:
                    ...
                    for orig_url, orig_title in zip(chunk, titles):
                        ...
                        canonical_url = urllib.parse.unquote(
                            f"https://en.wikipedia.org/wiki/{canonical_title.replace(' ', '_')}"
                        )
                        if canonical_url != orig_url:
                            canonical_map[canonical_url] = url_to_codes[orig_url]
        except Exception as e:
            if verbose:
                print(f"Wikipedia API error resolving canonicals: {e}")

    url_to_codes.update(canonical_map)
    return url_to_codes


def format_destinations_list(
    raw_destinations: list, airlines_destinations_map: dict, url_to_codes: dict
) -> list:
    """
        ...
        if isinstance(dest, dict):
            ...
            if d_url:
                if d_url in mapped_dict:
                    mapped_dict[d_url]["airlines"] = sorted(
                        list(
                            set(
                                mapped_dict[d_url].get("airlines", [])
                                + dest.get("airlines", [])
                            )
                        )
                    )
            ...
            if not codes:
                ...
                try:
                    ...
                    headers = {
                        "User-Agent": "wikipediaGATN/1.0 (julien.arino@example.com)"
                    }
                    title = urllib.parse.unquote(d_url.split("/wiki/")[-1])
                    r = requests.get(
                        f"https://en.wikipedia.org/w/api.php?action=query&titles={title}&redirects=1&format=json",
                        headers=headers,
                        timeout=5,
                    )
                    if r.status_code == 200:
                        ...
                        if "query" in res_json and "redirects" in res_json["query"]:
                            ...
                            target_url = urllib.parse.unquote(
                                f"https://en.wikipedia.org/wiki/{target_title.replace(' ', '_')}"
                            )
            ...
            if merge_key in mapped_dict:
                mapped_dict[merge_key]["airlines"] = sorted(
                    list(set(mapped_dict[merge_key]["airlines"] + op_airlines))
                )
            else:
                mapped_dict[merge_key] = {
                    ...
                    "codes": [
                        codes.get("iata", "iata code not found"),
                        codes.get("icao", "icao code not found"),
                        codes.get("gps", "gps code not found"),
                    ],
...
def infer_missing_geographic_data(data: dict) -> dict:
    ...
    if data.get("admin1_code"):
        ...
        if data.get("admin1_code") and data.get("admin1_name") == data.get(
            "admin1_code"
        ):
            try:
    ...
    if (
        not data.get("admin1_code")
        or len(str(data.get("admin1_code"))) > 3
        or not data.get("country_alpha3")
        or not data.get("admin2_name")
    ):
...
def compare_airports_with_ourairports(output_csv: Optional[str] = None) -> str:
    ...
    with open(airports_info_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            if row.get("iata"):
                known_iatas.add(row["iata"].strip().upper())
            if row.get("icao"):
                known_icaos.add(row["icao"].strip().upper())
    ...
    with open(ourairports_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if (
                (iata and iata in known_iatas)
                or (icao and icao in known_icaos)
                or (wiki_link in known_wikis)
                or (wiki_end in known_wikis)
            ):
                continue
...
def find_active_missing_airports(
    ...
    if output_csv is None:
        ...
        output_csv = os.path.join(
            TEMP_RESULTS_DIR, "missing_from_ourairports_active.csv"
        )

    if not os.path.exists(input_csv):
        raise FileNotFoundError(
            f"{input_csv} does not exist. Run compare_airports_with_ourairports() first."
        )
    ...
    def check_active(airport):
        ...
        try:
            headers = {
                "User-Agent": "wikipediaGATN/0.1.0 (Global Air Transportation Networks research; julien.arino@umanitoba.ca)"
            }
            res = requests.get(url, headers=headers, timeout=10)
            res.raise_for_status()
            html = res.text.lower()
            if (
                'id="airlines_and_destinations"' in html
                or 'id="passenger"' in html
                or 'id="cargo"' in html
                or 'id="airlines"' in html
            ):
                return airport
        except Exception:
            pass
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if processed % 100 == 0 or processed == total:
                print(
                    f"Processed {processed}/{total} pages. Found {len(active_airports)} active so far...",
                    end="\\r",
                    flush=True,
                )
