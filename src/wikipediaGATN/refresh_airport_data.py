...
def check_needs_refresh(file_paths: list[str], verbose: bool = False) -> list[str]:
    ...
    if verbose:
        print(
            f"Batch-checking Wikidata edit timestamps for {len(titles)} Wikipedia pages..."
        )
            ...
            for page in pages.values():
                ...
                if orig_title:
                    ...
                    if remote_dt > local_info["local_dt"]:
                        ...
                        if verbose:
                            print(
                                f"  ↻ {orig_title} needs refresh (remote {remote_dt} > local {local_info['local_dt']})"
                            )

        except Exception as exc:
            if verbose:
                print(
                    f"Error checking timestamps for chunk starting at index {i}: {exc}"
                )
...
def refresh_airport_file(
    ...
    if local_only:
        ...
        mapped_destinations = format_destinations_list(
            data.get("destinations", []), data.get("airlines_destinations", {}), url_map
        )
        ...
        mapped_destinations = format_destinations_list(
            new_data.get("destinations", []),
            new_data.get("airlines_destinations", {}),
            url_map,
        )
        mapped_destinations_cargo = format_destinations_list(
            new_data.get("destinations_cargo", []),
            new_data.get("airlines_destinations_cargo", {}),
            url_map,
        )
        ...
        wikitext, dt_wikidata = fetch_wikipedia_airport_wikitext(
            link=url, verbose=verbose
        )
        ...
        if not ad_map_wikitext["passenger"] and not ad_map_wikitext["cargo"]:
            ...
            ad_map = fetch_wikipedia_airlines_destinations(
                link=url, html_content=html_content, verbose=verbose
            )
        else:
        ...
        if unresolved:
            ...
            for i in range(0, len(unresolved), 50):
                ...
                titles = [
                    urllib.parse.unquote(url.split("/wiki/")[-1]) for url in chunk
                ]
                    ...
                    if r.status_code == 200:
                        ...
                        if "query" in res_json:
                            ...
                            for orig_url, orig_title in zip(chunk, titles):
                                canonical_title = title_to_canonical.get(
        ...
        mapped_destinations = format_destinations_list(
            destinations, airlines_destinations, url_map
        )
        mapped_destinations_cargo = format_destinations_list(
            destinations_cargo, airlines_destinations_cargo, url_map
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Refresh airport JSON data from Wikipedia."
    )
    parser.add_argument(
        "--target",
        type=str,
    ...
    if (
        isinstance(args.target, list)
        and len(args.target) == 1
        and args.target[0] == "all"
    ):
        args.target = "all"
