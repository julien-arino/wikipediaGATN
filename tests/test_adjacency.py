...
def _write_connections(public_dir: Path, rows: list) -> Path:
    """Write a minimal ``outbound_connections.csv`` and return its path."""
    csv_path = public_dir / "outbound_connections.csv"
...
class TestErrors:
    ...
    def test_raises_when_csv_missing(self, public_dir):
        """FileNotFoundError when outbound_connections.csv is absent."""
        with pytest.raises(FileNotFoundError, match="outbound_connections.csv"):
    ...
    def test_raises_when_csv_empty(self, public_dir):
        ...
        (public_dir / "outbound_connections.csv").write_text(
